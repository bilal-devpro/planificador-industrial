...
const { initDatabase } = require('./database-local');
...
initDatabase()
  .then(() => console.log("Base de datos SQLite lista"))
...
app.use('/api/produccion', require('./routes/produccion-local'));
app.use('/api/lineas', require('./routes/lineas'));

app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', message: 'Backend SQLite funcionando' });
});

app.listen(PORT, '0.0.0.0', () => {
  console.log(`🚀 Backend SQLite escuchando en puerto ${PORT}`);
});